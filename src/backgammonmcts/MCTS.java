...
public class MCTS {
    ...
    double midanchor = 0;
    //Blitz variables:
    double attack = 0.11526573538241869; //Variable that intensifies importance of attacks
    double safeAttack = 0.5936466883524378; //'''''''' of safe attacks (don't leave singles after attack)
    double endPlay = -0.9741008222267975; //'''''''' of not reaching endgame state
    double vurn = -1.1434114484624203; //'''''''' of vurnerability
    boolean endgame = false; //Boolean that turns true if no enemy pieces can attack friendly pieces anymore (for BLITZ)
    double offBearable = 0.6085679373798933; //Variable that intensifies importance of bearing off pieces
    double numFren0 = -0.7222640775713567; //'''''''' of number of friendlies that are in the moved position
    double numFren1 = 1.4006812380151201;
    double numFren2 = 1.2662092801833666;
    double numFren3 = 1.3748327701324374;
    double numFren4 = -0.30752241189789364;
    
    public MCTS() {
        tree = new Tree();
    }

	public State doStep(State currentState, String variant) {
		int n;
		switch (variant) {
			case "MostVisitedPruned":
				n = this.MCTSmostvisitedpruned(currentState, 1, 0, 3000);
				break;
			case "MostVisited":
				n = this.MCTSmostvisited(currentState, 50);
				break;
			case "HighestScore":
				n = this.MCTShighestscore(currentState, 50);
				break;
			case "HighestScorePruned":
				n = this.MCTShighestscorepruned(currentState, 1, 0, 50);
				break;
			default:
				n = -1;
				System.out.println("Everything is wrong. What are you doing? Go kill yourself. Also the argument to doStep" +
						" is not recognized.");
				break;
		}

		if (n == -1)
			return new State(currentState, new int[]{});
		int[] move = this.tree.root.state.movelist.get(n);

		return new State(currentState, move);
	}
    ...
    public int[] Blitz(State s) {
    	...
    	//Else continue with blitz strategy
	    	...
	    	double[] blitzScore = new double[s.getmovelist().size()]; //The score of each move, the move with the highest score gets returned.
	    	...
	    	for(int i=0;i<s.getmovelist().size();i++) {
	    		...
	    		if(r.getwhite() && !endgame) {
	    			...
	    			for(int j=1;j<r.getmovelist().get(i).length;j+=2) {
	    				...
	    				if((r.getmovelist().get(i)[j]) > 0) {
	    					...
	    					if(tempBoard[r.getmovelist().get(i)[j]] == -1) {
	    						blitzScore[i] = blitzScore[i] + attack * ((r.getmovelist().get(i)[j]));
	    					}
	    					...
	    					if((j+2)<r.getmovelist().get(i).length && r.getmovelist().get(i)[j] == r.getmovelist().get(i)[j+2]
	    						...
	    						blitzScore[i] = blitzScore[i] + safeAttack * ((r.getmovelist().get(i)[j]));
	    					}
	    					//If move follows up in an endgame state, blitzScore is reduced
	    					if(checkEndgame(r.getboard().board)) {
	    						blitzScore[i] = blitzScore[i] - blitzScore[i]/endPlay;
	    					}
	    				}
	    			}
	    			...
	    			for(int j=0;j<r.getmovelist().get(i).length;j+=2) {
	    				...
	    				if(tempBoard[r.getmovelist().get(i)[j]] == 1) {
    						blitzScore[i] = blitzScore[i] - vurn;
    					}
	    			}
	    		}
	    		//If the player is black
	    			...
	    			for(int j=1;j<r.getmovelist().get(i).length;j+=2) {
	    				...
	    				if((r.getmovelist().get(i)[j]) < 26) {
	    					...
	    					//according to the position of said checker (checkers closer to enemy's home give more points)
	    					if(tempBoard[r.getmovelist().get(i)[j]] == 1) {
	    						blitzScore[i] = blitzScore[i] + attack * ((1-(r.getmovelist().get(i)[j]/24.0))*24);
	    					}
	    					//If 2 checkers go to the same position AND attack an enemy checker, the score will be incremented as
	    					//it takes an enemy to the bar and create a safe position for itself
	    					if((j+2)<r.getmovelist().get(i).length && r.getmovelist().get(i)[j] == r.getmovelist().get(i)[j+2]
	    							&& tempBoard[r.getmovelist().get(i)[j]] == 1) {
	    						blitzScore[i] = blitzScore[i] + safeAttack * ((1-(r.getmovelist().get(i)[j]/24.0))*24);
	    					}
	    					//If move follows up in an endgame state, blitzScore is reduced
	    					if(checkEndgame(r.getboard().board)) {
	    						blitzScore[i] = blitzScore[i] - blitzScore[i]/endPlay;
	    	...
	    	for(int i=0;i<blitzScore.length;i++) {
	    		...
	    		if(max[0] < blitzScore[i]) {
	    			max[0] = (int)blitzScore[i]; //Cast double to int to avoid conflicts with int-array
	    			max[1] = i;
	    		}
	    	}
    ...
    //Method that sets the variables of blitz
    public void setBlitz(double attack, double safeAttack, double endPlay, double vurn, double offBearable, double numFren0, 
    		double numFren1, double numFren2, double numFren3, double numFren4) {
    	this.attack = attack;
    	this.safeAttack = safeAttack;
    	this.endPlay = endPlay;
    	this.vurn = vurn;
    	this.offBearable = offBearable;
    	this.numFren0 = numFren0;
    	this.numFren1 = numFren1;
    	this.numFren2 = numFren2;
    	this.numFren3 = numFren3;
    	this.numFren4 = numFren4;
    }
    
    //Method that gets the variables of blitz in an double array
    public double[] getBlitz() {
    	double[] ray = new double[10];
    	ray[0] = this.attack;
    	ray[1] = this.safeAttack;
    	ray[2] = this.endPlay;
    	ray[3] = this.vurn;
    	ray[4] = this.offBearable;
    	ray[5] = this.numFren0;
    	ray[6] = this.numFren1;
    	ray[7] = this.numFren2;
    	ray[8] = this.numFren3;
